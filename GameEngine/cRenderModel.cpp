...
void cRenderModel::DrawMesh(cLoadModels* mesh, glm::vec3 position, GLuint shaderProgram)
{
	mesh->pMeshTransform.x = position.x;
	mesh->pMeshTransform.y = position.y;
	mesh->pMeshTransform.z = position.z;

	mesh->bIsWireframe = false;

	Render(shaderProgram, mesh);
}
