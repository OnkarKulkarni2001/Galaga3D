...
void cPhysicsUpdated::CalculateAABB(cLoadModels& model)
{
	glm::vec3 min = model.pTransformedVertices[0].transformedVertex;
	glm::vec3 max = model.pTransformedVertices[0].transformedVertex;

	for (int vertexIndex = 0; vertexIndex != model.numberOfVertices; vertexIndex++) {
		min = glm::min(min, model.pTransformedVertices[vertexIndex].transformedVertex);
		max = glm::max(max, model.pTransformedVertices[vertexIndex].transformedVertex);
	}

	// Define an sAABB structure to store the AABB for this model
	cLoadModels::sAABB modelAABB;
	modelAABB.max = max;
	modelAABB.min = min;

	// Calculate and store the size, height, and width of the AABB
	modelAABB.size.x = max.x - min.x;  // Width (X-axis)
	modelAABB.size.y = max.y - min.y;  // Height (Y-axis)
	modelAABB.size.z = max.z - min.z;  // Depth (Z-axis)
	modelAABB.center = (min + max) / 2.0f;
...
void cPhysicsUpdated::CalculateBoundingSpheres(cLoadModels& model)
{
	//pBoundingSpheres = new sBoundingSphere[scene.pModels.size()];
	glm::vec3 center(0.0f);
	float distance(0.0f);
	...
	for (int vertexIndex = 0; vertexIndex != model.numberOfVertices; vertexIndex++) {
		center.x += model.pTransformedVertices[vertexIndex].transformedVertex.x;
		center.y += model.pTransformedVertices[vertexIndex].transformedVertex.y;
		center.z += model.pTransformedVertices[vertexIndex].transformedVertex.z;
		//center += model.position;
	}
	center.x /= model.numberOfVertices;
	center.y /= model.numberOfVertices;
	center.z /= model.numberOfVertices;
	//center /= model.numberOfVertices;

	for (int vertexIndex = 0; vertexIndex != model.numberOfVertices; vertexIndex++) {
		distance = glm::distance(center, glm::vec3(model.pTransformedVertices[vertexIndex].transformedVertex.x,
			model.pTransformedVertices[vertexIndex].transformedVertex.y,
			model.pTransformedVertices[vertexIndex].transformedVertex.z));
		//distance = glm::distance(center, model.position);
	...
	model.modelSphere.center = center;
	model.modelSphere.radius = maxDistance;
}
...
bool cPhysicsUpdated::CheckBoundingSphereCollision(cLoadModels::sBoundingSphere& sphere1, cLoadModels::sBoundingSphere& sphere2)
{
	float distance = glm::distance(sphere1.center, sphere2.center);
	if (distance <= (sphere1.radius + sphere2.radius)) {
		return true;		// they will collide
...
glm::vec3 cPhysicsUpdated::GetAABBCollisionPoint(cLoadModels::sAABB& aabb1, cLoadModels::sAABB& aabb2)
{
...
bool cPhysicsUpdated::CheckAABBCollision(cLoadModels::sAABB& aabb1, cLoadModels::sAABB& aabb2)
{

	if (aabb1.max.x > aabb2.min.x && aabb1.min.x < aabb2.max.x &&
		aabb1.max.y > aabb2.min.y && aabb1.min.y < aabb2.max.y &&
		aabb1.max.z > aabb2.min.z && aabb1.min.z < aabb2.max.z) {
		//sAABBAABB_Collision collision;
		//collision.collidingAABBs[0] = &aabb1;
		//collision.collidingAABBs[1] = &aabb2;
		//vecCollidingAABBs.push_back(collision);
		return true;
	}
	return false;
}
...
bool cPhysicsUpdated::CheckCollision(cLoadModels::sAABB& aabb1, cLoadModels::sAABB& aabb2)
{
	vecCollidingSpheres.clear();
	vecCollidingAABBs.clear();
	vecCollidingTriangles.clear();
...
//bool cPhysicsUpdated::CheckCollision(std::vector<cLoadModels> vecA, std::vector<cLoadModels> vecB)
//{
//	for (int modelIndex = 0; modelIndex != vecA.size(); modelIndex++) {
//		CalculateAABB(vecA[modelIndex]);
//	}
//	for (int modelIndex = 0; modelIndex != vecB.size(); modelIndex++) {
//		CalculateAABB(vecB[modelIndex]);
//	}
//
//	for (int modelIndex = 0; modelIndex != vecA.size(); modelIndex++) {
//		for (int model2Index = 0; model2Index != vecB.size(); model2Index++) {
//			CheckAABBCollision(vecA[modelIndex].modelAABB, vecB[model2Index].modelAABB);
//		}
//	}
//	return false;
//}
