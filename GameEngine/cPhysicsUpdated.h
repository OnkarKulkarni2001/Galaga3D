...
public:
	...
	struct sAABBAABB_Collision {
		cLoadModels* model;
		cLoadModels::sAABB* collidingAABBs[2];
	};
	...
	void CalculateAABB(cLoadModels& model);
	void CalculateBoundingSpheres(cScene& scene);
	void CalculateBoundingSpheres(cLoadModels& model);
	bool CheckBoundingSphereCollision(sBoundingSphere& sphere1, sBoundingSphere& sphere2);
	bool CheckBoundingSphereCollision(cLoadModels::sBoundingSphere& sphere1, cLoadModels::sBoundingSphere& sphere2);
	bool CheckAABBCollision(sAABB& aabb1, sAABB& aabb2);
	glm::vec3 GetAABBCollisionPoint(cLoadModels::sAABB& aabb1, cLoadModels::sAABB& aabb2);
	bool CheckAABBCollision(cLoadModels::sAABB& aabb1, cLoadModels::sAABB& aabb2);
	//bool CheckCollision(cScene& scene);
	bool CheckCollision(cLoadModels::sAABB& aabb1, cLoadModels::sAABB& aabb2);
